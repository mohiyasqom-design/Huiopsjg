...
const ImageGenerator = () => {
  ...
  return React.createElement(
    ...
    React.createElement('h2', { className: 'text-2xl font-bold text-center text-slate-800' }, 'ایجاد تصویر از طریق دستور متنی'),

    // Prompt input
    React.createElement(
      'div',
      null,
      React.createElement('label', { htmlFor: 'prompt-generate', className: 'block text-sm font-medium text-slate-600 mb-2' }, 'دستور خلاقانه شما'),
    ...
    React.createElement(
      'div',
      null,
      React.createElement('label', { className: 'block text-sm font-medium text-slate-600 mb-2' }, 'نسبت تصویر'),
      React.createElement(
        'div',
        { className: 'grid grid-cols-3 gap-2 rounded-lg bg-slate-100 p-1' },
    ...
    React.createElement(
      ...
      isLoading ? React.createElement(LoadingSpinner, null) : 'ایجاد کن'
    ),
    ...
    React.createElement(
      'div',
      { className: `relative w-full ${aspectRatioStyles[aspectRatio]} max-h-[75vh] mx-auto bg-slate-100 rounded-lg flex items-center justify-center border-2 border-dashed border-slate-300 transition-all duration-300` },
      isLoading
        ? React.createElement(
            'div',
            { className: 'text-center text-slate-500' },
            React.createElement(LoadingSpinner, { large: true }),
            React.createElement('p', { className: 'mt-2' }, 'در حال خلق شاهکار شما...')
          )
        : generatedImage
          ? React.createElement(
              React.Fragment,
              null,
              React.createElement('img', { src: generatedImage, alt: 'Generated', className: 'object-contain w-full h-full rounded-lg' }),
              React.createElement(
                'button',
                {
          ...
          : React.createElement('p', { className: 'text-slate-400' }, 'تصویر ساخته شده شما در اینجا نمایش داده می‌شود')
    )
  );
};
